...
func (s *scanner) maybeParseFile(
	...
	// 如果不是打包模式的话，对于 injected files 就跳过 import path resolution，以免报错
	// 让开发者产生迷惑
	skipResolve := false
...
func (s *scanner) addEntryPoints(entryPoints []EntryPoint) []graph.EntryPoint {
	...
	for i := range entryPoints {
		...
		dir := s.fs.Dir(absPath)
		base := s.fs.Base(absPath)
		// 如果 input 没带 relative path 的标记，什么是 relative path 的标记？比如 . 开头的路径
		// `esbuild app --bundle --outfile=out.js`
		// 命令中的 app 对于插件们来说，看起来就是 node_modules 下的包，这就不对了，所以下面这段代码就是把
		// entries 中看起来像是 node_modules 的、其实是相对路径的，加上 . 变成相对路径
		if entries, err, originalError := s.fs.ReadDirectory(dir); err == nil {
			if entry, _ := entries.Get(base); entry != nil && entry.Kind(s.fs) == fs.FileEntry {
	...
	// 这里就是贯彻最大程度并行，对每个 entry 都开个 goroutine 来处理
	...
	for i, info := range entryPointInfos {
		...
		for _, resolveResult := range info.results {
			...
			// maybeParseFile 内部也是并行的，结果会输入到 s.resultChannel 中
...
func (cache *runtimeCache) parseRuntime(options *config.Options) (source logger.Source, runtimeAST js_ast.AST, ok bool) {
	...
	(func() {
		...
		if cache.astMap != nil {
			// go 的 map 可以用 struct 作为 key，并且 key 的比较是比较 struct 的内容而非 reference
			// 所以用上面直接初始化出来的 key 来做检索 is reasonable
